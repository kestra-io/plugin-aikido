...
class ListRepositoriesTest {
    ...
    void run(WireMockRuntimeInfo wireMockRuntimeInfo) throws Exception {
        ...
        stubFor(get(urlPathEqualTo("/api/public/v1/repositories/code")).willReturn(okJson("""
            [{"id":1,"name":"pied-piper/rest-api","provider":"github","external_repo_id":"123","branch":"main","last_scanned_at":1700000000}]
            """)));

        var task = ListRepositories.builder()
        ...
        assertThat(output.getRows().getFirst().getName(), is("pied-piper/rest-api"));
