...
@Schema(
    ...
    description = "Downloads the license and package overview for a code repository and stores it to internal storage. Fails with an actionable message if the repository has no completed scan yet — run `ScanRepository` first."
)
@Plugin(
    examples = {
        @Example(
            ...
            code = """
                ...
                tasks:
                  - id: export_sbom
                    type: io.kestra.plugin.aikido.repositories.ExportRepositorySbom
...
public class ExportRepositorySbom extends AbstractAikidoTask implements RunnableTask<ExportRepositorySbom.Output> {
    ...
    public Output run(RunContext runContext) throws Exception {
        ...
        try (var client = client(runContext)) {
            String body;
            try {
                body = client.getText("/repositories/code/" + rRepositoryId + "/licenses/export", query, "repositories:read");
            } catch (AikidoApiException e) {
                if (e.getMessage() != null && e.getMessage().contains("HTTP 404")) {
                    throw new IllegalStateException("No SBOM available for Aikido repository '" + rRepositoryId + "' — it has likely not completed a scan yet. Run ScanRepository first.", e);
                }
                throw e;
            }
            ...
            var uri = runContext.storage().putFile(
                ...
                "aikido-repository-sbom" + extension
            );
